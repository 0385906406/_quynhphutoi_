// Xoá 1 bình luận bài viết — tác giả hoặc admin.
import { NextResponse } from "next/server";
import { getCurrentUser } from "@/lib/admin";
import { isAdmin } from "@/lib/users";
import { deleteNewsComment } from "@/lib/news-social";
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ slug: string; id: string }> }) {
  ...
  const deleted = await deleteNewsComment(id, user._id!.toString(), isAdmin(user));
