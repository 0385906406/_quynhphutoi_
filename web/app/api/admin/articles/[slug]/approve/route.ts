...
export async function POST(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  void logActivity({ userId: g.user._id!.toString(), userName: g.user.name, userEmail: g.user.email, userRole: g.user.role ?? "admin", category: "admin", action: approved ? "article.approve" : "article.reject", target: { type: "bài viết", id: slug, label: article.title }, success: true });
