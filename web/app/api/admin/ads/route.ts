...
export async function GET() {
  ...
  return NextResponse.json({
    ads: rows.map((a) => ({
      id: a._id!.toString(), advertiser: a.advertiser, title: a.title, imageDesktop: a.imageDesktop,
      imageMobile: a.imageMobile ?? null, linkUrl: a.linkUrl, placement: a.placement, weight: a.weight,
...
export async function POST(req: Request) {
  ...
  if (!/^https?:\/\//i.test(linkUrl)) return NextResponse.json({ error: "Link đích phải bắt đầu bằng http(s)://" }, { status: 400 });
  ...
  const ad = await createAd({
    ...
    linkUrl, placement: placement as AdPlacement,
