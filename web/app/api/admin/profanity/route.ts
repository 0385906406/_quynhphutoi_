  ...
  try {
    ...
    if (listSource) {
      const added = await addProfanityWords(listSource, !!b.accentInsensitive);
        void logActivity({ userId: g.user._id!.toString(), userName: g.user.name, userEmail: g.user.email, userRole: g.user.role ?? "admin", category: "admin", action: "profanity.bulk-add", success: true });
    ...
    const created = await addProfanityWord({
      ...
      void logActivity({ userId: g.user._id!.toString(), userName: g.user.name, userEmail: g.user.email, userRole: g.user.role ?? "admin", category: "admin", action: "profanity.add", success: true });
