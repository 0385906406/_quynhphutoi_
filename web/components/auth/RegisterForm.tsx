...
export function RegisterForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.status === 429) {
      router.push(`/bi-chan?seconds=${data.retryAfter ?? 60}&from=dang-ky`);
      return;
    }
    if (!res.ok) {
    ...
    <form className="form-panel" onSubmit={handleSubmit} noValidate>
      ...
      <div className="field-group">
        <label className="field-label" htmlFor="reg-name">
          Họ và tên <CharCount value={name} max={80} />
        </label>
        <div className="input-wrap">
          <input type="text" id="reg-name" placeholder="Nguyễn Văn A" autoComplete="name" required maxLength={80} value={name} onChange={(e) => setName(e.target.value)} />
      ...
      <div className="field-group">
        ...
        <div className="input-wrap">
          <input type="email" id="reg-email" placeholder="ten@quynhphu.vn" autoComplete="email" required value={email} onChange={(e) => setEmail(e.target.value)} />
      ...
      <div className="field-group">
        ...
        <div className="input-wrap input-wrap--has-eye">
          <input type={showPw ? "text" : "password"} id={pwId} placeholder="Tối thiểu 6 ký tự" autoComplete="new-password" required value={password} onChange={(e) => setPassword(e.target.value)} />
          <svg className="ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
      ...
      <div className="field-group">
        <label className="field-label" htmlFor={confirmId}>Nhập lại mật khẩu</label>
        <div className="input-wrap input-wrap--has-eye">
          <input type={showConfirm ? "text" : "password"} id={confirmId} placeholder="••••••••" autoComplete="new-password" required value={confirm} onChange={(e) => setConfirm(e.target.value)} />
          <svg className="ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
            <rect x="3" y="11" width="18" height="11" rx="2" />
            <path d="M7 11V7a5 5 0 0 1 10 0v4" />
          </svg>
      ...
      <div className="row-extra">
        <label className="remember">
          ...
          Tôi đồng ý với <Link href="/dieu-khoan" style={{ color: "var(--g3)" }}>điều khoản sử dụng</Link>
        </label>
      </div>
      ...
      <div className="social-row">
        <button type="button" className="btn-social" onClick={() => { window.location.href = "/api/auth/google"; }}>
        ...
        <button type="button" className="btn-social" onClick={() => { window.location.href = "/api/auth/microsoft"; }}>
