            ...
            <tbody>
              {pg.paged.map((r) => (
                <tr key={r.slug}>
                  ...
                  <td>
                    <button type="button" className="qp-admin-link-btn qp-clip" title={r.title} style={{ fontWeight: 700, color: "var(--color-navy)" }} onClick={() => setDetail(r)}>{r.title}</button>
                    {r.featured && <> <span className="qp-badge-g4">Nổi bật</span></>}
                    <span className="qp-clip--sm type-body-small text-muted" title={r.sub}>{r.sub}</span>
                  </td>
