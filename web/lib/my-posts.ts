...
export type MyPostSection = "tim-do-roi" | "viec-lam" | "mua-ban";
...
export async function getMyPosts(userId: string): Promise<MyPost[]> {
  const [lf, jobs, ads] = await Promise.all([listMyPosts(userId), listMyJobs(userId), listMyClassifieds(userId)]);
